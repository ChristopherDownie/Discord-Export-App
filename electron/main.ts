...
function downloadFile(url: string, destPath: string): Promise<{ success: boolean; error?: string }> {
...
ipcMain.handle('list-reports', async () => {
  try {
    ...
    // Sort newest first
    summaries.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
...
ipcMain.handle('load-report', async (_event, reportId: string) => {
  try {
...
ipcMain.handle('delete-report', async (_event, reportId: string) => {
  try {
    const filePath = path.join(getReportsDir(), reportId + '.json');
